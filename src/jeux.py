...
def joueur_vs_ia():
    ...
    short_hide()
    while (check_game_end('Ordi', J, FlotteJoueur, nb_tour) == False) :
        nb_tour = nb_tour +1
        ...
        short_hide()
        if (check_game_end(NomJoueur, M, FlotteIA, nb_tour) == False) :
            print("\x1b[0mAu tour de l'IA d'attaquer.")
...
def deux_joueurs() :
    ...
    long_hide()
    while (check_game_end(NomJoueur2, MatriceJ1, FlotteJoueur1, nb_tour) == False) :
        nb_tour = nb_tour +1
        print("\x1b[0;31mVotre flotte ",NomJoueur1, " : ")
        plot_flotte_grid(create_grid(),FlotteJoueur1)
        ...
        if (check_game_end(NomJoueur1, MatriceJ2, FlotteJoueur2, nb_tour) == False) :
            long_hide()
            print("\x1b[0;34mVotre flotte ",NomJoueur2," : ")
            plot_flotte_grid(create_grid(),FlotteJoueur2)
